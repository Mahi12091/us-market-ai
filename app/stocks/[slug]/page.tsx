...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = stock.description ?? `${stock.company_name} (${stock.symbol}) stock price, technical analysis, quantitative predictions, market news and research.`;
  return {
    title: `${stock.company_name} (${stock.symbol}) Stock Analysis & Price Prediction`,
    description,
    alternates: { canonical: `/stocks/${slug}` },
    robots: { index: true, follow: true },
    openGraph: { title: `${stock.company_name} (${stock.symbol}) Stock Analysis`, description, type: 'article', url: `/stocks/${slug}` },
...
export default async function StockPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="stock-page">
    ...
    <div className="container">
      ...
      {(related ?? []).length ? <section className="section"><div className="section-head"><div><div className="eyebrow">RELATED RESEARCH</div><h2>More {stock.sector ?? 'market'} stocks</h2><p className="muted">Explore other active stocks in the same sector.</p></div></div><div className="grid grid-4">{related?.map(r => <a className="panel indicator-card" href={`/stocks/${r.slug}`} key={r.id}><span>{r.symbol}</span><strong>{r.company_name}</strong><small>{r.market_cap ? `$${Number(r.market_cap).toLocaleString()}M market cap` : 'US stock'}</small></a>)}</div></section> : null}
