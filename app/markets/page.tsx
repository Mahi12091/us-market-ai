...
export default async function MarketsPage() {
  ...
  const [{ data: proxyStocks }, { data: stocks }] = await Promise.all([
  ...
  const proxyMap = new Map((proxyStocks ?? []).map(s => [s.symbol,{...s,quote:quoteMap.get(s.id)}]));
  const validQuotes = (stocks ?? []).map(s => ({...s,quote:quoteMap.get(s.id)})).filter(s => s.quote?.change_percent != null);
