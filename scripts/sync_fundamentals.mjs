...
function mapRowsToTickerMap(raw){
  // company_tickers_exchange.json is a columnar SEC file:
  // { fields: ['cik','name','ticker','exchange'], data: [[cik,name,ticker,exchange], ...] }
  if(Array.isArray(raw?.data)&&Array.isArray(raw?.fields)){
    const index=new Map(raw.fields.map((field,i)=>[String(field).toLowerCase(),i]));
    const tickerIndex=index.get('ticker');
    const cikIndex=index.get('cik');
    if(tickerIndex!=null&&cikIndex!=null){
      return new Map(raw.data.map(row=>[
        String(row?.[tickerIndex]??'').trim().toUpperCase(),
        String(row?.[cikIndex]??'').padStart(10,'0')
      ]).filter(([ticker,cik])=>ticker&&cik!=='0000000000'));
    }
  }
  if(Array.isArray(raw)){
    return new Map(raw.map(x=>[
      String(x?.ticker||'').trim().toUpperCase(),
      String(x?.cik||x?.cik_str||'').padStart(10,'0')
    ]).filter(([ticker,cik])=>ticker&&cik!=='0000000000'));
  }
  return new Map(Object.values(raw||{}).map(x=>[
    String(x?.ticker||'').trim().toUpperCase(),
    String(x?.cik||x?.cik_str||'').padStart(10,'0')
  ]).filter(([ticker,cik])=>ticker&&cik!=='0000000000'));
}
...
async function loadTickerMap(){
  try{
    ...
    const map=mapRowsToTickerMap(raw);
    if(map.size<100)throw new Error(`unexpected mapping size ${map.size}`);
    return map;
  }catch(first){
    try{
      const raw=await secFile('/files/company_tickers.json');
      const map=mapRowsToTickerMap(raw);
      if(!map.size)throw new Error('empty ticker mapping');
      return map;
    }catch(second){
...
try{
  ...
  console.log(`Loaded ${tickerMap.size} SEC ticker/CIK associations.`);
}catch(e){
