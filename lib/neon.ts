...
export class NeonQuery<T = Record<string, unknown>> implements PromiseLike<QueryResult<T>> {
  ...
  eq(column: string, value: unknown) {
    this.filters.push({ sql: `${quoteIdent(column)} = $VALUE`, values: [value] });
    return this;
  }
  ...
  is(column: string, value: null | boolean) {
    this.filters.push(
      value === null
        ? { sql: `${quoteIdent(column)} IS NULL`, values: [] }
        : { sql: `${quoteIdent(column)} IS ${value ? 'TRUE' : 'FALSE'}`, values: [] },
    );
    return this;
  }

  in(column: string, values: unknown[]) {
    this.filters.push({ sql: `${quoteIdent(column)} IN ($VALUES)`, values });
    return this;
  }
  ...
  or(expression: string) {
    const terms = expression
      ...
      .map((term) => {
        ...
        const value = rawValue.startsWith('%') || rawValue.endsWith('%') ? rawValue : rawValue;
        const sqlOp = op === 'eq' ? '=' : op === 'neq' ? '<>' : op === 'gt' ? '>' : op === 'gte' ? '>=' : op === 'lt' ? '<' : op === 'lte' ? '<=' : 'ILIKE';
        return { sql: `${quoteIdent(column)} ${sqlOp} $VALUE`, value };
      });
    this.filters.push({
      sql: `(${terms.map((t) => t.sql).join(' OR ')})`,
  ...
  private buildWhere() {
    ...
    for (const filter of this.filters) {
      if (filter.values.length > 1 && filter.sql.includes('$VALUE')) {
        const replacements = filter.values.map((value) => {
          values.push(value);
          return `$${++index}`;
        });
    ...
    try {
      ...
      if (this.action === 'select') {
        ...
        if (this.orderBy.length) {
          q += ' ORDER BY ' + this.orderBy.map((item) =>
            `${quoteIdent(item.column)} ${item.ascending ? 'ASC' : 'DESC'}${item.nullsFirst == null ? '' : item.nullsFirst ? ' NULLS FIRST' : ' NULLS LAST'}`
          ).join(', ');
        }
      ...
      if (this.action === 'delete') {
        return { data: (await sql(`DELETE FROM ${this.table}${where.where} RETURNING *`, where.values)) as T[], error: null };
      }

      if (this.action === 'update') {
        const row = this.payload[0] ?? {};
        const values = [...where.values];
        const assignments = Object.keys(row).map((key) => {
          values.push(row[key]);
