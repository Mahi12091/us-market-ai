...
export class NeonQuery<T = Record<string, any>, Single extends boolean = false>
  ...
  order(column: string, options?: { ascending?: boolean; nullsFirst?: boolean }) { this.orderBy.push({ column, ascending: options?.ascending !== false, nullsFirst: options?.nullsFirst }); return this; }
  ...
  private buildWhere() {
    let index = 0; const values: unknown[] = []; const parts: string[] = [];
    for (const filter of this.filters) {
      if (filter.values.length > 1 && filter.sql.includes('$VALUE')) {
        ...
        parts.push(filter.sql.replace(/\$VALUE/g, () => replacements.shift() ?? 'NULL')); continue;
      }
      if (filter.sql.includes('$VALUES')) {
        const placeholders = filter.values.map((value) => { values.push(value); return `$${++index}`; }).join(',');
        parts.push(filter.sql.replace('$VALUES', placeholders || 'NULL')); continue;
      }
      if (filter.sql.includes('$VALUE')) { values.push(filter.values[0]); parts.push(filter.sql.replace('$VALUE', `$${++index}`)); continue; }
  ...
  async execute(): Promise<Single extends true ? SingleResult<T> : ManyResult<T>> {
    try {
      ...
      if (this.action === 'delete') {
        const rows = (await sql.query(`DELETE FROM ${this.table}${where.where} RETURNING *`, where.values)) as T[];
        return { data: rows, error: null } as Single extends true ? SingleResult<T> : ManyResult<T>;
      }
      if (this.action === 'update') {
        const row = this.payload[0] ?? {}; const values = [...where.values];
        const assignments = Object.keys(row).map((key) => { values.push(row[key]); return `${quoteIdent(key)} = $${values.length}`; }).join(', ');
        const rows = (await sql.query(`UPDATE ${this.table} SET ${assignments}${where.where} RETURNING *`, values)) as T[];
        return { data: rows, error: null } as Single extends true ? SingleResult<T> : ManyResult<T>;
      }
      if (!this.payload.length) return { data: [], error: null } as Single extends true ? SingleResult<T> : ManyResult<T>;
      const keys = [...new Set(this.payload.flatMap((row) => Object.keys(row)))]; const values: unknown[] = [];
      ...
      const rows = (await sql.query(`${q} RETURNING *`, values)) as T[];
      return { data: rows, error: null } as Single extends true ? SingleResult<T> : ManyResult<T>;
    } catch (error) {
      return { data: null, error: error instanceof Error ? error : new Error(String(error)) } as Single extends true ? SingleResult<T> : ManyResult<T>;
    }
  }
...
export function createDatabaseClient() {
  return { from<T = Record<string, any>>(table: string) { return new NeonQuery<T>(table); } };
}
