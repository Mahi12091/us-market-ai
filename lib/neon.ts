...
export class NeonQuery<T = Record<string, unknown>> implements PromiseLike<{ data: T[] | null; error: Error | null }> {
  private table: string; private selectColumns = '*'; private filters: Filter[] = []; private orderBy?: { column: string; ascending: boolean }; private limitCount?: number; private offsetCount?: number; private action: 'select' | 'insert' | 'upsert' | 'update' | 'delete' = 'select'; private payload: Record<string, unknown>[] = []; private conflictColumns: string[] = [];
  ...
  is(column: string, value: null | boolean) { this.filters.push(value === null ? { sql: `${quoteIdent(column)} IS NULL`, values: [] } : { sql: `${quoteIdent(column)} IS ${value ? 'TRUE' : 'FALSE'}`, values: [] }); return this; }
  ...
  private buildWhere() { let index = 0; const values: unknown[] = []; const parts = this.filters.map((filter) => { if (filter.sql.includes('$VALUES')) { const placeholders = filter.values.map((value) => { values.push(value); return `$${++index}`; }).join(','); return filter.sql.replace('$VALUES', placeholders || 'NULL'); } if (filter.sql.includes('$VALUE')) { values.push(filter.values[0]); return filter.sql.replace('$VALUE', `$${++index}`); } return filter.sql; }); return { where: parts.length ? ` WHERE ${parts.join(' AND ')}` : '', values }; }
  async execute(): Promise<{ data: T[] | null; error: Error | null }> {
    try {
      const where = this.buildWhere();
      if (this.action === 'select') { let q = `SELECT ${columns(this.selectColumns)} FROM ${this.table}${where.where}`; if (this.orderBy) q += ` ORDER BY ${quoteIdent(this.orderBy.column)} ${this.orderBy.ascending ? 'ASC' : 'DESC'}`; if (this.limitCount != null) q += ` LIMIT ${Math.max(0, this.limitCount)}`; if (this.offsetCount != null) q += ` OFFSET ${Math.max(0, this.offsetCount)}`; return { data: (await sql(q, where.values)) as T[], error: null }; }
      if (this.action === 'delete') return { data: (await sql(`DELETE FROM ${this.table}${where.where} RETURNING *`, where.values)) as T[], error: null };
      if (this.action === 'update') { const row = this.payload[0] ?? {}; const values = [...where.values]; const assignments = Object.keys(row).map((key) => { values.push(row[key]); return `${quoteIdent(key)} = $${values.length}`; }).join(', '); return { data: (await sql(`UPDATE ${this.table} SET ${assignments}${where.where} RETURNING *`, values)) as T[], error: null }; }
      if (!this.payload.length) return { data: [], error: null };
      const keys = [...new Set(this.payload.flatMap((row) => Object.keys(row)))]; const values: unknown[] = []; const tuples = this.payload.map((row) => `(${keys.map((key) => { values.push(row[key] ?? null); return `$${values.length}`; }).join(',')})`).join(','); let q = `INSERT INTO ${this.table} (${keys.map(quoteIdent).join(',')}) VALUES ${tuples}`;
      if (this.action === 'upsert' && this.conflictColumns.length) { const updates = keys.filter((key) => !this.conflictColumns.includes(key)).map((key) => `${quoteIdent(key)} = EXCLUDED.${quoteIdent(key)}`).join(', '); q += ` ON CONFLICT (${this.conflictColumns.map(quoteIdent).join(',')}) DO ${updates ? `UPDATE SET ${updates}` : 'NOTHING'}`; }
...
export function createDatabaseClient() { return { from<T = Record<string, unknown>>(table: string) { return new NeonQuery<T>(table); } }; }
