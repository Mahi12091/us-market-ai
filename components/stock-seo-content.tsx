...
export default function StockSeoContent({ stock, quote, tech, predictions, fundamentals, earnings, news, results }: Props) {
  ...
  return <div className="seo-research-stack">
    ...
    <Card id="stock-price" eyebrow="STOCK / PRICE" title={`${symbol} Stock Price & Performance`}>
    ...
    <Card id="forecast" eyebrow="FORECAST / PREDICTION" title={`${symbol} Stock Forecast`}>
    ...
    <Card id="prediction" eyebrow="QUANTITATIVE MODEL" title={`${symbol} Stock Prediction`}>
    ...
    <Card id="52-week" eyebrow="52-WEEK DATA" title={`${symbol} 52 Week High & Low`}>
      <AI>Explain the current price position within the 52-week range using calculated historical highs/lows. Do not invent a range.</AI>
      <div className="seo-metric-grid"><div className="seo-metric"><span>52 Week High</span><strong>Derived from price history</strong></div><div className="seo-metric"><span>52 Week Low</span><strong>Derived from price history</strong></div><div className="seo-metric"><span>Current Price</span><strong>{money(quote?.price)}</strong></div></div>
      <h3>{symbol} 52 Week High Analysis</h3><AI>Explain distance from the verified 52-week high once calculated.</AI>
      <h3>{symbol} 52 Week Low Analysis</h3><AI>Explain distance from the verified 52-week low once calculated.</AI>
    </Card>
