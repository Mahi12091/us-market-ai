...
export default function StockSeoContent({ stock, quote, tech, predictions, fundamentals, earnings, news, results, article }: Props) {
  ...
  return <div className="seo-research-stack">
    <Card id="stock-price" eyebrow="STOCK / PRICE" title={`${symbol} Stock Price & Share Price`}>
      ...
      <h3>{symbol} Recent Price Performance</h3>
    ...
    <Card id="prediction-forecast" eyebrow="PREDICTION & FORECAST" title={`${symbol} Stock Prediction & Forecast`}>
      ...
      <h3>{symbol} Stock Forecast Next 7 Days</h3><ResearchText>Use the 7D model row when available. It represents a short-horizon quantitative estimate and should not be presented as a certain future price.</ResearchText>
      <h3>{symbol} Stock Forecast Next 30 Days</h3><ResearchText>Use the 30D model row when available. A longer horizon carries additional uncertainty because new market and company information can change the inputs.</ResearchText>
      <h3>{symbol} Stock Forecast This Week & This Month</h3><ResearchText>Weekly and monthly search queries are mapped to the corresponding short-horizon model outputs. US Market AI does not create a separate number when a validated model value is unavailable.</ResearchText>
    </Card>
    ...
    <Card id="technical-analysis" eyebrow="TECHNICAL ANALYSIS" title={`${symbol} Technical Analysis`}>
      ...
      <h3>{symbol} RSI, MACD & Moving Averages</h3>
      ...
      <h3>{symbol} Support and Resistance Levels</h3>
    ...
    <Card id="financials" eyebrow="FUNDAMENTALS" title={`${symbol} Financials, Revenue, EPS & ROE`}>
      ...
      <h3>{symbol} Revenue and Revenue Growth</h3><ResearchText>Revenue and revenue growth help describe the scale and direction of the business. Use the verified figures above when available; no missing growth rate is estimated.</ResearchText>
      <h3>{symbol} EPS, Free Cash Flow and ROE</h3><ResearchText>EPS, free cash flow and return on equity provide additional profitability and capital-efficiency context. Each metric should be interpreted with the company's industry and reporting period in mind.</ResearchText>
    </Card>

    <Card id="valuation" eyebrow="VALUATION" title={`${symbol} Valuation, PE Ratio & Forward PE`}>
      ...
      <h3>{symbol} Fair Value</h3><ResearchText>US Market AI does not label a missing fair-value estimate as a fact. If a validated valuation model is connected later, its methodology and assumptions can be displayed here.</ResearchText>
    </Card>

    <Card id="earnings" eyebrow="EARNINGS & DIVIDEND" title={`${symbol} Earnings, EPS Estimate & Dividend`}>
      <h3>{symbol} Earnings Date and Results</h3>
      ...
      <h3>{symbol} Earnings Forecast</h3><ResearchText>Earnings forecasts are shown only when verified earnings data is available. The stock-price prediction model is separate from an earnings consensus estimate.</ResearchText>
      <h3>{symbol} Dividend</h3><ResearchText>Dividend yield, amount, ex-dividend date and payment date are not inferred when the dividend feed is unavailable. A future dividend data connector can populate this section.</ResearchText>
    </Card>

    <Card id="company" eyebrow="COMPANY INFORMATION" title={`About ${company} (${symbol})`}>
    ...
    <Card id="long-term" eyebrow="LONG-TERM FORECAST" title={`${symbol} Long-Term Stock Forecast 2026–2050`}>
      <ResearchText>Long-term forecast searches such as {symbol} stock forecast 2026, 2027, 2030, 2035, 2040 and 2050 are grouped here. US Market AI will show a numerical value only when a dedicated validated long-horizon model supplies it; short-term predictions are not extrapolated into distant years.</ResearchText>
      <div className="seo-data-table"><table><thead><tr><th>Horizon</th><th>Status</th></tr></thead><tbody>{['2026','2027','2030','2035','2040','2050'].map(y => <tr key={y}><th>{symbol} Forecast {y}</th><td>Dedicated long-horizon model pending</td></tr>)}</tbody></table></div>
    </Card>
